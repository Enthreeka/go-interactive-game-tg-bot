...
func (b *Bot) CallbackStrings(callbackData string) (error, ViewFunc) {
	...
	case strings.HasPrefix(callbackData, "create_mailing"):
		callbackView, ok := b.callbackView["create_mailing"]
		if !ok {
			return ErrNotFound, nil
		}
		return nil, callbackView

	case strings.HasPrefix(callbackData, "update_message"):
		callbackView, ok := b.callbackView["update_message"]
		if !ok {
			return ErrNotFound, nil
		}
		return nil, callbackView
